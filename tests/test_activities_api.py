...
def test_fetch_page_returns_activities_and_usage() -> None:
    ...
    with httpx.Client(timeout=5.0) as client:
        page = fetch_page(
            client,
            access_token="bearer-xyz",  # ruff: ignore[hardcoded-password-func-arg] — fixture bearer token, not a real credential
            after=None,
            page=1,
            per_page=200,
        )
...
def test_fetch_page_raises_auth_error_on_401() -> None:
    ...
    with httpx.Client(timeout=5.0) as client, pytest.raises(AuthError):
        fetch_page(client, access_token="bad", after=None, page=1, per_page=200)  # ruff: ignore[hardcoded-password-func-arg] — fixture bearer token, not a real credential
...
def test_fetch_page_raises_rate_limited_on_429() -> None:
    ...
    with httpx.Client(timeout=5.0) as client, pytest.raises(RateLimited) as excinfo:
        fetch_page(client, access_token="t", after=None, page=1, per_page=200)  # ruff: ignore[hardcoded-password-func-arg] — fixture bearer token, not a real credential
