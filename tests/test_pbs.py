...
def test_compute_rows_empty_db_returns_empty(session: Session) -> None:
    rows = pbs.compute_rows(session, athlete_id=42)
    assert rows == []


def test_compute_rows_one_break_one_row(session: Session) -> None:
    store.upsert_activity(session, athlete_id=42, activity=_summary(1, "2024-04-15T08:00:00"))
    store.upsert_best_efforts(session, athlete_id=42, activity_id=1, efforts=[_effort("5K", 1100)])
    session.commit()
    rows = pbs.compute_rows(session, athlete_id=42)
...
def test_compute_rows_equalled_does_not_break(session: Session) -> None:
    store.upsert_activity(session, athlete_id=42, activity=_summary(1, "2024-04-15T08:00:00"))
    store.upsert_activity(session, athlete_id=42, activity=_summary(2, "2024-05-01T08:00:00"))
    store.upsert_best_efforts(session, athlete_id=42, activity_id=1, efforts=[_effort("5K", 1100)])
...
def test_compute_rows_three_distances_one_race_one_row(session: Session) -> None:
    store.upsert_activity(session, athlete_id=42, activity=_summary(1, "2024-04-15T08:00:00"))
    store.upsert_best_efforts(
        session,
        athlete_id=42,
...
def test_compute_rows_multiple_dates_newest_first(session: Session) -> None:
    for activity_id, date_local, time_s in [
        (1, "2023-04-15T08:00:00", 1200),
        (2, "2023-09-01T08:00:00", 1150),
        (3, "2024-04-15T08:00:00", 1100),
    ]:
        store.upsert_activity(session, athlete_id=42, activity=_summary(activity_id, date_local))
        store.upsert_best_efforts(
            session,
            athlete_id=42,
...
def test_compute_rows_running_best_fills_other_columns(session: Session) -> None:
    """A row's non-broken cells show the running best at that distance."""
    store.upsert_activity(session, athlete_id=42, activity=_summary(1, "2023-01-01T08:00:00"))
    store.upsert_best_efforts(session, athlete_id=42, activity_id=1, efforts=[_effort("5K", 1200)])
    store.upsert_activity(session, athlete_id=42, activity=_summary(2, "2024-04-15T08:00:00"))
    store.upsert_best_efforts(
        session,
        athlete_id=42,
        activity_id=2,
...
def test_compute_rows_scopes_by_athlete(session: Session) -> None:
    store.upsert_activity(session, athlete_id=1, activity=_summary(1, "2024-04-15T08:00:00"))
    store.upsert_best_efforts(session, athlete_id=1, activity_id=1, efforts=[_effort("5K", 1100)])
    session.commit()
    rows_other = pbs.compute_rows(session, athlete_id=2)
    rows_self = pbs.compute_rows(session, athlete_id=1)
